...
from transition_trigger.data import build_datasets, make_training_sampler
from transition_trigger.evaluate import evaluate_model
from transition_trigger.losses import transition_loss, resolve_pos_weight
from transition_trigger.model import TransitionTriggerHead


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a causal transition trigger.")
...
def main() -> int:
    ...
    model = TransitionTriggerHead(input_dim=train_dataset.input_dim, **config["model"]).to(args.device)
    ...
    for epoch in range(1, int(config["training"]["epochs"]) + 1):
        ...
        for batch in train_loader:
            ...
            loss = transition_loss(logits, labels, valid, config["training"], pos_weight=pos_weight)
