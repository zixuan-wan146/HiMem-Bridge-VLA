...
from himem_bridge_vla.training.stage1.batch_contract import validate_stage1_window_batch
...
from himem_bridge_vla.training.stage1.cli import build_arg_parser
from himem_bridge_vla.training.stage1.config import build_stage1_config
from himem_bridge_vla.training.stage1.loss import stage1_flow_matching_loss
from himem_bridge_vla.training.stage1.loop import _scatter_progress_state
from himem_bridge_vla.training.stage1.validators import enforce_stage1_contract, validate_stage1_cache_contract


REPO_ROOT = Path(__file__).resolve().parents[1]
TRAIN_STAGE1_SCRIPT = REPO_ROOT / "scripts" / "train_stage1.py"
...
def test_train_stage1_script_is_thin_launcher():
    ...
    assert "from himem_bridge_vla.training.stage1.cli import main" in source
...
def test_stage1_module_help_does_not_require_training_runtime():
    result = subprocess.run(
        [sys.executable, "-m", "himem_bridge_vla.training.stage1.cli", "--help"],
        cwd=REPO_ROOT,
        text=True,
        capture_output=True,
        check=False,
...
def test_build_stage1_config_uses_active_profile_and_cli_override():
    args = build_arg_parser().parse_args(
        [
            "--config",
            "configs/stage1/libero_10_direct_progress_w4.yaml",
            "--batch_size",
            "2",
        ]
    )
