...
def test_episode_offset_can_be_configured():
    config = LiberoClientConfig.from_env({"HIMEM_LIBERO_EPISODE_OFFSET": "7"})
...
def test_negative_episode_offset_is_rejected():
    try:
        LiberoClientConfig.from_env({"HIMEM_LIBERO_EPISODE_OFFSET": "-1"})
    except ValueError as exc:
        assert "HIMEM_LIBERO_EPISODE_OFFSET" in str(exc)
    else:
