...
```text
LIBERO / 单臂默认 d_a = 7
RMBench / 双臂 joint action 默认 d_a = 14
```
...
`Norm_a` 按 benchmark 和 action protocol 固定，LIBERO 与 RMBench 分别使用各自统计量。normalized executed actions、normalized target actions、IntentEncoder AE 训练数据必须保持相同 action 维度、相同关节/夹爪顺序、相同 min-max 或 mean-std 统计、相同 clipping 策略。
...
`use_order_loss` 的默认值是 `false`。order loss 是可选正则项，不进入当前 LIBERO 和 RMBench warm-up 主配置。
...
当前已完成 LIBERO 与 RMBench 的 progress-state warm-up 训练。所有训练均使用：
...
RMBench cache 只保存 pooled VL embedding：
...
```text
himem_bridge_vla/model/planner/progress_state.py
```
