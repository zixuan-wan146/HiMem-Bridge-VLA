...
本文记录当前 HiMem-Bridge-VLA 在 `libero_10` 上的训练 recipe。当前文档只锁定 Stage 1：基于 VLM token cache 的 action-side flow matching 训练。Stage 2 的 raw image / VLM finetune 设置后续单独补充。
...
对于第 `k` 个训练样本，当前低层时间步为 `t_k`，监督动作 chunk 为：
...
progress_planner:
  ...
  checkpoint: /root/autodl-tmp/runs/progress_warmup/libero_progress_state_planner_h32_r16_w4_bs12800_epval_v1/best.pt
  online_forward: true
```
...
如果显存和 loss 都稳定，可以提高到 `48`。如果训练不稳或 OOM，退回 `16`。

## 17. Eval And Checkpoint

当前 `scripts/train.py` 的 Stage 1 训练入口只实现 training loss 记录和 checkpoint 保存；它还没有内置 episode-level validation loop。因此当前 `step_best` 是按训练 loss 监控得到的 health checkpoint，不应被解释为 validation-best checkpoint。
