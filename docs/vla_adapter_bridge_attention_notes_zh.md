...
3. 它的 context 和 gating 设计对 HiMem-Bridge-VLA 有什么启发。
...
在我们的 HiMem-Bridge-VLA 设计中，如果 horizon 为 32，则更自然的是：
...
因此，HiMem-Bridge-VLA 后续 bridge-attn 更推荐的主线是让 32 个 noisy action tokens 直接读取 current VL、short memory、expanded plan slots 和 state。
