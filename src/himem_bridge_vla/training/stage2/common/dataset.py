...
from himem_bridge_vla.dataset.memory_replay_frames import MemoryReplayFrameReader, ReplayFrame
from himem_bridge_vla.dataset.memory_token_cache import _apply_token_cache_normalization
from himem_bridge_vla.dataset.memory_token_cache import _parse_token_cache_normalization
...
def prepare_stage2_dataset(config: dict[str, Any], *, repo_root: str | Path) -> "LiberoRawEpisodeSequenceDataset":
    ...
    dataset = LiberoRawEpisodeSequenceDataset(
        index_path,
    ...
    logging.info(
        "Loaded Stage2 raw LIBERO replay index: episodes=%s sequence_len=%s manifest=%s",
        len(dataset),
...
def prepare_stage2_dataloader(dataset: "LiberoRawEpisodeSequenceDataset", config: dict[str, Any]):
    try:
    ...
    dataloader = DataLoader(
        ...
        drop_last=True,
        collate_fn=collate_libero_raw_episode_sequences,
...
class LiberoRawEpisodeSequenceDataset:
    """MemoryVLA-style cheap group sampler over raw LIBERO episodes."""
    ...
    def __init__(
        ...
        *,
        ...
        if payload.get("format") != "libero_episode_replay_index":
            raise ValueError(f"expected libero_episode_replay_index, got {payload.get('format')!r}")
        benchmark = str(payload.get("benchmark", "LIBERO")).upper()
        if benchmark != "LIBERO":
            raise ValueError(f"LiberoRawEpisodeSequenceDataset requires LIBERO, got {benchmark!r}")
        ...
        self.data_root = Path(data_root or payload.get("libero_root") or self.index_path.parent).expanduser()
        self.normalization = normalization
        self.arm2stats_dict = None if normalization is None else dict(normalization["stats"])
        self.reader = MemoryReplayFrameReader(benchmark="LIBERO", data_root=self.data_root)
