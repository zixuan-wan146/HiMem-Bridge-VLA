...
run_step "Requirements policy audit" "$python_bin" scripts/quality/audit_requirements.py
run_step "Runtime environment check" "$python_bin" scripts/quality/check_runtime_environment.py
...
run_ruff
check_shell_syntax
run_step "Repository preflight" "$python_bin" scripts/quality/preflight.py
run_step "Bridge-HiMem config validation" "$python_bin" scripts/quality/validate_bridge_himem_configs.py
run_step "Training config validation" "$python_bin" scripts/quality/validate_training_configs.py
run_step \
  "Direct bridge inference smoke" \
  "$python_bin" scripts/quality/smoke_direct_bridge_inference.py --preset tiny --device cpu
run_step \
  "Direct bridge token-cache training smoke" \
  "$python_bin" scripts/quality/smoke_direct_bridge_token_cache_training.py --preset tiny --device cpu --steps 1 --batch-size 2
run_step "Benchmark inventory" "$python_bin" scripts/eval/inspect_benchmarks.py --allow-missing
run_step "LIBERO setup dry-run" env HIMEM_SETUP_LIBERO_DRY_RUN=1 scripts/setup/setup_libero_env.sh
run_step \
  "LIBERO checkpoint download dry-run" \
  env HIMEM_DOWNLOAD_LIBERO_CHECKPOINT_DRY_RUN=1 scripts/setup/download_libero_checkpoint.sh
run_step \
  "LIBERO smoke profile dry-run" \
  env HIMEM_LIBERO_DRY_RUN=1 HIMEM_LIBERO_PROFILE=configs/runtime/libero_profiles/smoke.env \
  scripts/eval/run_libero_smoke.sh
run_step \
  "LIBERO eval profile dry-run" \
  env HIMEM_LIBERO_DRY_RUN=1 HIMEM_LIBERO_PROFILE=configs/runtime/libero_profiles/full_eval.env \
  scripts/eval/run_libero_eval.sh
run_step \
  "LIBERO experiment init dry-run" \
  "$python_bin" scripts/eval/init_libero_experiment.py \
  --dry-run \
...
if [ "${HIMEM_CHECK_SKIP_COMPILE:-0}" = "1" ]; then
  ...
  run_step \
    ...
    "$python_bin" -m compileall -q src/himem_bridge_vla evaluations scripts tests
fi
