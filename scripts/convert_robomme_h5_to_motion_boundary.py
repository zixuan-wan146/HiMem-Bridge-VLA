...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert RoboMME H5 trajectories into motion_boundary segmented parquet."
    )
    ...
    parser.add_argument(
        "--include-terminal",
        action="store_true",
        help="Also add info/is_completed frames to the boundary jsonl.",
    )
...
def main() -> int:
    ...
    for task_index, (task, h5_path) in enumerate(dataset_specs):
        ...
        for key in ("episodes", "frames", "boundaries", "terminal_events", "dropped_video_demo_frames"):
    ...
    with h5py.File(h5_path, "r") as h5_file:
        ...
        for episode_index, episode_name in enumerate(episode_names):
            ...
            rows, episode_boundaries, episode_stats = convert_episode(
                ...
                next_segment_id=len(boundaries),
            )
            if not rows:
                continue
...
def convert_episode(
    ...
    *,
    ...
    return (
        ...
        {
            "frames": len(rows),
            "boundaries": len(boundary_frames),
