...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert RMBench HDF5 demo_clean trajectories into transition_trigger parquet."
    )
    parser.add_argument(
        "--input-root",
        required=True,
    ...
    parser.add_argument("--output-root", required=True, help="Output transition_trigger dataset root.")
    parser.add_argument("--tasks", nargs="+", default=DEFAULT_RMBENCH_TASKS)
    parser.add_argument("--max-episodes", type=int, default=None)
    parser.add_argument(
        "--include-terminal",
        action="store_true",
        help="Also include final language segment ends as terminal transition events.",
    )
...
def main() -> int:
    ...
    for task_index, task in enumerate(args.tasks):
        task_stats = convert_task(
            ...
            next_segment_id=lambda: len(transitions),
            transitions=transitions,
        )
        stats["datasets"].append(task_stats)
        for key in ("episodes", "frames", "transitions", "terminal_events", "duration_mismatches"):
    ...
    *,
    ...
    for segment_id, segment in enumerate(segment_ranges):
        ...
        transition_rows.append(
            {
                "segment_id": next_segment_id + len(transition_rows),
                "episode_id": episode_id,
                "task": task,
